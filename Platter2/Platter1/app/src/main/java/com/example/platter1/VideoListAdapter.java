...
public class VideoListAdapter extends RecyclerView.Adapter<VideoListAdapter.videoHolder> {
    ...
    public VideoListAdapter(Context context, OnItemClickListener listener){
        this.context=context;
        this.listener=listener;
        list= new ArrayList<>();
    }

    public void setVideolist(ArrayList<Video_info> list) {
        this.list = list;
    }

    @Override
    public videoHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new videoHolder(LayoutInflater.from(context).inflate(R.layout.layout_videolist,parent,false));
    }
    ...
    public void onBindViewHolder(videoHolder holder, @SuppressLint("RecyclerView") int position) {
        ...
        holder.tv_year.setText(String.valueOf(list.get(position).year));
        holder.tv_description.setText(list.get(position).description);
        Glide.with(context).load(Public_data.url+"cloud_img/"+list.get(position).video_id+".jpg").into(holder.iv);
